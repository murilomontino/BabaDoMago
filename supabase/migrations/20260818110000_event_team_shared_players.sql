alter table public.championship_event_team_players
	drop constraint if exists championship_event_team_players_event_id_player_id_key;

create unique index if not exists championship_event_team_players_team_player_idx
	on public.championship_event_team_players (team_id, player_id);
...
select
	...
	(elem.value::text)::bigint = t.template_goalkeeper_id
		and t.template_goalkeeper_id is distinct from 0
from public.championship_event_teams t
cross join lateral jsonb_array_elements(t.template_player_ids) as elem
join public.championship_players p
	on p.id = (elem.value #>> '{}')::bigint
...
begin
	...
	for i in 0 .. team_count - 1 loop
		...
		foreach player_id in array player_ids loop
			if player_id = any (seen_team_players) then
				raise exception 'duplicate player' using errcode = '23505';
			end if;

			seen_team_players := seen_team_players || player_id;
