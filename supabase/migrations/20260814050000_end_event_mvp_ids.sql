drop function if exists public.end_championship_event(bigint, jsonb);

create or replace function public.end_championship_event(
	event_id bigint,
	present_player_ids jsonb default null,
	mvp_player_ids jsonb default null
)
returns jsonb
language plpgsql
security definer
...
begin
	...
	if mvp_player_ids is not null then
		if public.championship_actor_role(event.championship_id) not in ('owner', 'captain') then
			raise exception 'not allowed' using errcode = '42501';
		end if;

		if jsonb_typeof(mvp_player_ids) is distinct from 'array' then
			raise exception 'invalid attendance' using errcode = '23514';
		end if;

		if exists (
			select 1
			from jsonb_array_elements(mvp_player_ids) elem
			where jsonb_typeof(elem) is distinct from 'number'
		) then
			raise exception 'invalid attendance' using errcode = '23514';
		end if;

		select coalesce(array_agg(elem::bigint order by elem::bigint), '{}')
		into mvp_ids
		from jsonb_array_elements_text(mvp_player_ids) as elem;
		...
		select coalesce(array_agg(a.player_id order by a.player_id), '{}')
		into auto_mvp_ids
		from public.championship_event_attendance a
		where a.event_id = event.id
			and a.is_mvp;

		if mvp_ids is distinct from auto_mvp_ids then
...
revoke all on function public.end_championship_event(bigint, jsonb, jsonb) from public;
grant execute on function public.end_championship_event(bigint, jsonb, jsonb) to authenticated;
