-- Congela a projeção da próxima ao abrir o perfil (e reusa na presença).

alter table public.championship_players
	add column if not exists rating_projected_next numeric null;

comment on column public.championship_players.rating_projected_next is
	'Projeção da próxima rodada congelada (perfil); copiada para attendance.rating_projected.';
...
begin
	...
	select p.rating_projected_next
	into pending
	from public.championship_players p
	...
	if open_event.id is not null then
		...
		if attendance.id is not null then
			...
			projected := coalesce(
				...
				public.championship_attendance_rating_projected_fill(
					open_event.id,
					player.id,
					attendance.rating
				)
