alter table public.championship_players
	add column if not exists assisted_goals integer not null default 0;
...
begin
	...
	set
		...
		assisted_goals = coalesce(totals.assisted_goals, 0),
	...
	left join (
		select
			...
			sum(
				case
					when open_match.event_id is null then a.assisted_goals
					else coalesce(ended_assisted_goals.n, 0)
				end
			)::integer as assisted_goals,
			sum(
				case
