...
declare
	...
	existing_match_player public.championship_event_match_players%rowtype;
...
begin
	...
	if has_existing and not existing_match_player.is_substituted then
	...
	incoming_display_name := coalesce(nullif(btrim(player.nickname), ''), player.display_name);

	if has_existing then
		update public.championship_event_match_players mp
		set
			team_id = set_championship_event_match_player.team_id,
			slot = set_championship_event_match_player.slot,
			is_substituted = false,
			is_goalkeeper = set_championship_event_match_player.slot = 0,
			display_name = incoming_display_name
		where mp.id = existing_match_player.id;
