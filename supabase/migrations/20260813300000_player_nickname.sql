...
alter table public.championship_players
	drop constraint if exists championship_players_nickname_check;

alter table public.championship_players
	add constraint championship_players_nickname_check
	check (
		nickname is null
		or char_length(btrim(nickname)) between 1 and 40
	);
...
create or replace function public.update_player_nickname(
	player_id bigint,
	nickname text
)
returns jsonb
...
revoke all on function public.update_player_nickname(bigint, text) from public;
grant execute on function public.update_player_nickname(bigint, text) to authenticated;
