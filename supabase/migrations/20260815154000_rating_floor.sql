create or replace function public.championship_player_rating_apply(
	rating numeric,
	delta numeric
)
returns numeric
...
as $$
	select case
		when rating = 0 and round((rating + delta)::numeric, 1) <= 0 then 0
		else least(100, greatest(0.1, round((rating + delta)::numeric, 1)))
	end;
$$;
...
revoke all on function public.championship_player_rating_apply(numeric, numeric) from public;
