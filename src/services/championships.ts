...
function asPlayer(value: unknown): ChampionshipPlayer {
	...
	const goalkeeperRating = Number(
		row.goalkeeper_rating ?? PLAYER_RATING.default,
	);
	if (
	...
	const hiddenStrength = Number(row.hidden_strength ?? PLAYER_RATING.default);
	if (
		!Number.isFinite(hiddenStrength) ||
		hiddenStrength < PLAYER_RATING.min ||
		hiddenStrength > PLAYER_RATING.max
	) {
		throw new Error("player: invalid payload");
	}

	const hiddenGoalkeeperStrength = Number(
		row.hidden_goalkeeper_strength ?? PLAYER_RATING.default,
	);
	if (
		!Number.isFinite(hiddenGoalkeeperStrength) ||
		hiddenGoalkeeperStrength < PLAYER_RATING.min ||
		hiddenGoalkeeperStrength > PLAYER_RATING.max
	) {
		throw new Error("player: invalid payload");
	}
